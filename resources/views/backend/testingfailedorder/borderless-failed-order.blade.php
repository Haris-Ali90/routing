...
@section('title', 'Toronto Failed Orders ')
...
@section('inlineJS')
    ...
    <script>
                        ...
                        buttons: {
                            'confirm': {
                                ...
                                action: function () {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "{{ URL::to('backend/borderless/create/all/order')}}",
                                        data: {ids: del_id},
        ...
        $(document).on('click', '.sprint', function () {
            ...
            if ($(this).context.innerText == 'Create Order') {
                ...
                if (name.replace(/\s/g, "") == "") {
                    ...
                    $.confirm({
                        title: 'Alert',
                        content: 'Customer Phone No Is Required.',
                        icon: 'fa fa-question-circle',
                        animation: 'scale',
                        closeAnimation: 'scale',
                        opacity: 0.5,
                ...
                // }
                    ...
                    $.confirm({
                        title: 'Alert',
                        content: 'Customer Address Is Required.',
                        icon: 'fa fa-question-circle',
                        animation: 'scale',
                        closeAnimation: 'scale',
                        opacity: 0.5,
        ...
        function sendDataSprint(data, context) {
            //   $(".loader").show();
            //            $(".loader-background").show();
            ...
            $.ajax({
                url: "{{ URL::to('backend/borderless/failed/orders')}}",
                type: 'POST',
                beforeSend: function (request) {
                    return request.setRequestHeader('X-CSRF-Token', "{{ csrf_token() }}");
                },
                ...
                success: function (data) {
                    ...
                    if (str_data.status == 200) {
                        $.confirm({
                            title: 'Alert',
                            content: str_data.message,
                            icon: 'fa fa-question-circle',
                            animation: 'scale',
                            closeAnimation: 'scale',
                            opacity: 0.5,
    ...
    <script>
        $(document).ready(function () {
            $(".dselect").click(function () {
                ...
                if (del_id.length == 0) {
                    ...
                    $.confirm({
                        title: 'A secure action',
                        content: 'Are you sure you want to Create Order ?',
                        icon: 'fa fa-question-circle',
                        animation: 'scale',
                        closeAnimation: 'scale',
                        opacity: 0.5,
                        buttons: {
                            'confirm': {
                                ...
                                action: function () {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "{{ URL::to('backend/borderless/task')}}",
                                        data: {ids: del_id},
                                        beforeSend: function (request) {
                                           console.log(request)
        ...
        $(document).on('click', '.nonsprint', function () {
            $(this).closest("tr").find("input, span.view").toggle();
            var error = null;
            if ($(this).context.innerText == 'Create Task') {
                setupRows($(this).closest("tr"));
                currentRow = $(this).closest("tr");
                var tracking_num = currentRow.find("td:eq(2)").text();
                var vendor_id = currentRow.find("td:eq(3)").text();
                var name = currentRow.find("td:eq(4)").text();
                var phone = currentRow.find("td:eq(5)").text();
                var email = currentRow.find("td:eq(6)").text();
                var addressline = currentRow.find("td:eq(7)").text();
                var addressline2 = currentRow.find("td:eq(8)").text();
                var merchant_id = currentRow.find("td:eq(10)").text();
                var confirm_signature = currentRow.find("td:eq(11)").text();
                var start_time = currentRow.find("td:eq(12)").text();
                var end_time = currentRow.find("td:eq(13)").text();
                var weight = currentRow.find("td:eq(14)").text();
                var sprint_id = currentRow.find("td:eq(16)").text();
                var del_id = $(this).attr("data-id");
                var data = new FormData();
                data.append('id', del_id);
                data.append('vendor_id', vendor_id);
                data.append('tracking_num', tracking_num);
                data.append('name', name);
                data.append('phone', phone);
                data.append('email', email);
                data.append('line', addressline);
                data.append('line2', addressline2);
                data.append('merchant_order_number', merchant_id);
                data.append('confirm_signature', confirm_signature);
                data.append('start_time', start_time);
                data.append('end_time', end_time);
                data.append('weight', weight);
                data.append('sprint_id', sprint_id);
                var regex = /^[a-zA-Z\s]+$/;
                var m_regex = /^[1-9]\d{9}$/;
                var p_regex = new RegExp(/^[ABCEGHJKLMNPRSTVXY]\d[ABCEGHJKLMNPRSTVWXYZ]( )?\d[ABCEGHJKLMNPRSTVWXYZ]\d$/i);
                if (name.replace(/\s/g, "") == "") {
            ...
            if ($(this).context.innerText == "Edit") {
                if (error == null) {
                    console.log(error);
                    $(this).context.innerText = "Create Task";
                }
            }
        });
        ...
        function sendData(data, context) {
            ...
            $.ajax({
                ...
                // dataType: "json",
                success: function (data) {
                    var str_data = JSON.parse(data);
                    console.log(str_data.status);
                    $(".loader").hide();
                    $(".loader-background").hide();
                    if (str_data.status == 200) {
                        $.confirm({
                    ...
                    //     //     }
                    //     // });
                    //     // $('.div-alert').html("<div class='alert alert-error alert-block' style ='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'>×</button><strong>"+str_data.error+"</strong></div>");
                    //
                    //     //     $(".loader").hide();
                    //     // $(".loader-background").hide();
                    //     //     bootprompt.alert(data['error']);
...
@section('content')
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_title">
                            <form id="filter" style="padding: 10px;" action="" method="get">
                                ...
                                <div class="row">
                                   <div class="col-lg-3">
                                   <input id="vendor_id" name="vendor_id" type="text" style="margin-bottom:10px"
                                           placeholder="Vendor ID" class="form-control">
                                   </div>
                                    <div class="col-lg-3">
                                    <input id="tracking_id" name="tracking_id" type="text" style="margin-bottom:10px"
                                           placeholder="Tracking ID" class="form-control">
                                    </div>

                                    <div class="col-lg-3">
                                    <button  id="search" type="submit" class="btn sub-ad green-gradient">
                                        Filter
                                    </button>
                                    </div>

                                    <div class="col-lg-3 d-flex justify-content-end">
                                    <button  type="button" class="dselect btn sub-ad btn-primary">Create
                                        Selected Order
                                    </button>
                                    </div>
    ...
    background-color: #000000ba; display: none"
                                ...
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    <tr>
                                        <th class="checkboxcss"><input class='checkboxx' type='checkbox' name='check'
                                                                       id="checkAll"></th>
                                        <th>Id</th>
                                        <th>Tracking ID</th>
                                        <th>Vendor ID</th>
                                    ...
                                    <td><input type="text" style="display: none;"
                                               value="<?php echo  $failed_orders->customer_name;?>">
                                        <span class="view"><?php echo $failed_orders->customer_name; ?></span></td>
                                    <td><input type="text" style="display: none;"
                                               value="<?php echo  $failed_orders->customer_number;?>">
                                        <span class="view"><?php echo $failed_orders->customer_number; ?></span></td>
                                    <td><input type="email" style="display: none;"
                                               value="<?php echo  $failed_orders->customer_email;?>">
                                        <span class="view"><?php echo $failed_orders->customer_email; ?></span></td>
                                    <td><input type="text" style="display: none;" class="google-address"
                                               value="<?php echo  $failed_orders->address;?>">
                                        <span class="view"><?php echo $failed_orders->address; ?></span></td>
                                    <td><input type="text" style="display: none;" class="google-address"
                                               value="<?php echo  $failed_orders->address_line_2;?>">
                                    ...
                                    echo "<td>" . $failed_orders->sprint_id . "</td>";
                                    echo "<td>" . $failed_orders->created_at . "</td>";
                                    echo "<td>";
                                    // if(can_access_route(['borderless-failed-order.post'],$userPermissoins)){
                                    if ($failed_orders->sprint_id == '') {
                                        echo "<button  data-id='" . $failed_orders->id . "' class='btn green-gradient toggle-button sprint'>Edit</button>";
                                    } else {
                                        echo "<button  data-id='" . $failed_orders->id . "' class='btn green-gradient sub-ad toggle-button nonsprint'>Edit</button>";
                                    }
