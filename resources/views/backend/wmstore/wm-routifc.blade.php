    ...
    <div class="right_col" role="main">
        <div class="">
           ...
           foreach ($routes as $route) {
               ...
               echo "<td>".$route->store_name ."(". $route->store_num .")". "</td>";
               ...
               echo "<td><button class='details green-gradient btn' data-route-id=" . $route->route_id . " data-joey-id=" . $route->joey_id . " title='Details'>D</button>
                <a class=' orange-gradient btn' target='_blank' href='../wm/route/" . $route->route_id . "/edit' title='Edit Route'>E</a> 
        ...
        for (var i = 0; i < data['data'].length; i++) {
          ...
          //  if (data[i]['type'] == "dropoff") {
                ...
                if(mapmarker==1)
                    ...
                    map: map,
                    icon: icon_marker,
                    title1:"WR-"+data['data'][i][k]['route_id']+"<br>CR-"+data['data'][i][k]['sprint_id']+"<br>("+data['data'][i][k]['address']+")",
                    //"http://maps.google.com/mapfiles/kml/paddle/"+arr[i%30]+".png",
                    //"https://assets.joeyco.com/images/marker/marker_red"+(k+1)+".png",
                    title:   "WR-"+data['data'][i][k]['route_id']+"\nCR-"+data['data'][i][k]['sprint_id']+"\n("+data['data'][i][k]['address']+")"
                });
...
function fullmap() {
    ...
    <?php    if(empty($_REQUEST['date'])){
                ...
                $.ajax({
                    ...
                    url : '../../backend/wm/allroute/location/joey?date=<?php echo $date; ?>',
                    type : 'GET',
                    dataType:'json',
                          ...
                          else
                          {
                            var icon_marker="http://maps.google.com/mapfiles/kml/paddle/"+arr[i%30]+".png";
                                ...
                                var marker = new google.maps.Marker({
                                    ...
                                    map: map,
                                    icon:icon_marker,
                                    title1:"WR-"+data['data'][i][j]['route_id']+"<br>CR-"+data['data'][i][j]['sprint_id']+"<br>("+data['data'][i][j]['address']+")",
                                    title: "WR-"+data['data'][i][j]['route_id']+"\nCR-"+data['data'][i][j]['sprint_id']+"\n("+data['data'][i][j]['address']+")"
                                });
                                google.maps.event.addListener(marker, 'click', (function(marker, j) {
                                    return function() {
                                        infowindow.setContent(marker.title1);
...
<script>
    ...
    function mapCreate(data){
        ...
        for (var i = 0; i < data.length; i++) {
            // if (data[i]['type'] == "dropoff") {
                ...
                maplocation[i]=latlng;

                bounds.extend(latlng);
                var icon_marker="";
                    if(data[i]['type'] == "dropoff")
                    {
                        icon_marker="https://assets.joeyco.com/images/map/pins/numbers/small/orange/1.png";
                    }
                    else
                    {
                        markerorderdetail=markerorderdetail+"WR-" + data[i]['route_id']+"-"+(i+1)+"(CR-"+data[i]['sprint_id']+")\n";
                        markerdeltail=markerdeltail+"WR-" + data[i]['route_id']+"-"+(i+1)+"(CR-"+data[i]['sprint_id']+")<br>";
                        icon_marker="https://assets.staging.joeyco.com/images/map/pins/letters/small/orange/A.png";
                    }
                    ...
                    map: map,
                    icon: icon_marker,
                    //"https://assets.joeyco.com/images/marker/marker_red"+j+".png",
                    title:   markerorderdetail + "\n(" + data[i]['address'] + ")"
                });
            
                
                google.maps.event.addListener(marker, 'click', (function(marker, i) {
                    return function() {
                        infowindow.setContent(markerdeltail + "<br>(" + data[i]['address'] + ")");
                        infowindow.open(map, marker);
                    }
                })(marker, i));
                    ...
                    icon: icon_marker,
                    //"https://assets.joeyco.com/images/marker/marker_red"+j+".png",
                    title:   "WR-" + data[i]['route_id'] +"-"+(i+1)+ " (CR-"+data[i]['sprint_id']+")\n(" + data[i]['address'] + ")"
                });
            
                
                google.maps.event.addListener(marker, 'click', (function(marker, i) {
                    return function() {
                        infowindow.setContent("WR-" + data[i]['route_id'] +"-"+(i+1)+ " (CR-"+data[i]['sprint_id']+")\n(" + data[i]['address'] + ")");
                        infowindow.open(map, marker);
                    }
                })(marker, i));
        ...
        var roadTripCoordinates = maplocation;
    ...
    $(document).on('click', '.details', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.forEach(function(route) {
                    var heading = "<h4>WR-" + route.route_id +'-'+route.ordinal+ "</h4>";
        ...
        $('#details').modal();
        $('#details .order-id').text("WR-" + routeId);
    ...
    $(document).on('click', '.transfer', function(e) {
        ...
        $('#transfer').modal();
        $('#transfer .order-id').text("WR-" + routeId);
