        ...
        <div class="">
            ...
            <div class="page-title">
                <div class="title_left amazon-text">
                    <!-- <h3>Task Detail List<small></small></h3> -->
                </div>
            </div>
...
{{--                                        <th>Joey Id</th>--}}
                                    ...
                                    <tbody>
                                    @foreach($sprintDetails as $key => $detail)
                                        @php
                                             $joeyId = (isset($detail->joey)) ? $detail->joey->id : null;
                                        @endphp
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>{{ $detail->pickup_time }}</td>
                                            <td>{{ 'CR-'.$detail->sprint_id }}</td>
{{--                                            <td>{{ (isset($detail->first_name) && isset($detail->last_name)) ? $detail->first_name .' '.$detail->last_name : 'N/A' }}</td>--}}
                                            <td>
                                                @if($key == 0)
                                                    Pickup
                                                @else
                                            ...
                                            <td>{{ $detail->merchant_order_num}}</td>
                                            <td>{{ $detail->apart_no }}</td>
                                            <td>{{ $detail->street }}</td>

                                            <td>{{ $test[$detail->status_id] }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
    ...
    <div id="un-assign" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form action='../../haillify/booking/unassign' method='get' class='UnAssignBooking'>
    ...
    <div id="cancel" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form action='../../haillify/booking/cancel' method='get' class='CancelBooking'>
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    ...
    <div id="reject" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form action='../../haillify/booking/reject' method='get' class='RejectBooking'>
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        function transfer() {
            ...
            if(joey_id  == 'undefined'){
                ...
                $.ajax({
                    ...
                    data:{joey_id : joey_id,sprint_id:sprint_id},
                    success: function (data) {
                        if(data.status)
                        {
