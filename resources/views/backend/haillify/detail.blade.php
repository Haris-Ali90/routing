...
@section('content')
        ...
        <div class="">
            ...
            <div class="page-title">
                <div class="title_left amazon-text">
                    <!-- <h3>Delivery Detail List<small></small></h3> -->
                </div>
            </div>
                        ...
                        <div class="x_content">
                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                    @php
                                        $delivered_status = [17, 113, 114, 116, 117, 118, 132, 138, 139, 144, 145];
                                    @endphp
                                    @foreach($haillifyBookingDetails as $key => $detail)
                                        @php
                                            if(isset($detail->sprint->joey)){
                                                if($detail->sprint->joey->id != null){
                                                    $joey = \App\Joey::find($detail->sprint->joey->id);
                                                }
                                            }
                                        @endphp
                                        <tr>
                                            ...
                                            <td>{{ $detail->haillify_id }}</td>
                                            <td>{{ $detail->delivery_id }}</td>
                                            <td>
                                                @if(isset($joey))
                                            ...
                                            <td>{{ 'CR-'.$detail->sprint_id }}</td>
                                            <td>{{ $test[$detail->sprint->status_id] }}</td>
                                            <td>
                                                <a class=' orange-gradient btn control-size' target='_blank' href='../../haillify/booking/sprint/{{$detail->sprint_id}}' title='Detail'>Detail</a>
                                                @if(!in_array($detail->sprint->status_id, $delivered_status))
                                                    @if(isset($detail->sprint->joey))
                                                        @if($detail->sprint->joey->id != null)
                                                            <button class='un-assign  black-gradient btn control-size' style="padding: 6px 6px;" data-sprint-id="{{$detail->sprint_id}}" data-delivery-id="{{$detail->delivery_id}}" title='Un Assign'>Un Assign</button>
                                                        @endif
                                                    @endif
                                                    <button class='update-status  green-gradient btn control-size' data-delivery-id="{{$detail->delivery_id}}" data-sprint-id="{{ $detail->sprint_id }}" title='update status' style="width: 100px;">Update Status</button>
                                                    <button class='cancel  red-gradient btn control-size' data-delivery-id="{{$detail->delivery_id}}" data-sprint-id="{{$detail->sprint_id}}" title='cancel'>Cancel</button>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
    ...
    {{--  Cancel Booking Confirmation Model --}}
    <div id="cancel" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form action='../../haillify/booking/cancel' method='get' class='CancelBooking' style="padding: 15px">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" id="delivery_id" name="delivery_id" value="">
                    <input type="hidden" id="sprint_id" name="sprint_id" value="">
                    <div class="form-group">
                        <p><b>Are you sure you want to Cancel Booking ?</b></p>
                    </div>
    ...
    <div id="update-status" class="modal" role="dialog" aria-hidden="true">
            ...
            <div class='modal-dialog'>
                <div style="padding: 20px;" class='modal-content'>
                    ...
                    <form action='' method="POST">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <label style="width: 100% !important;">Please Select a Status</label>
                        <select name="status_id" id="status_id" class="form-control chosen-select s">
                            @foreach($status as $key => $oc)
                                <option value="{{ $key }}">{{ $oc }}</option>
                            @endforeach
                        </select>
                        <input type="hidden" name="status-delivery_id" id="status-delivery_id">
                        <input type="hidden" name="status-sprint_id" id="status-sprint_id">
                        <br>
                        <a type="submit" data-selected-row="false"  onclick="updateStatus()" class="btn green-gradient transfer-model-btn">update</a>
    ...
    <div id="un-assign" class="modal" style="display: none">
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form action='../../haillify/booking/un_assign/driver' method='get' class='UnAssignBooking' style="padding: 15px;">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" id="sprint_id" name="sprint_id" value="">
                    <input type="hidden" id="un_assign_delivery_id" name="un_assign_delivery_id" value="">
                    <div class="form-group">
    ...
    <script>
        ...
        $('.CancelBooking').submit(function (event) {
            ...
            data.append('delivery_id', $('input[name=delivery_id]').val());
        ...
        $(document).on('click', '.update-status', function(e) {
            ...
            var deliveryId = this.getAttribute('data-delivery-id');
            var sprintId = this.getAttribute('data-sprint-id');
            $('#status-delivery_id').val(deliveryId);
            $('#status-sprint_id').val(sprintId);
            $('#update-status').modal();
        });

        //update status of haillify booking
        function updateStatus() {
            let status_id = $('#status_id').val();
            let delivery_id = $('#status-delivery_id').val()
            let sprint_id = $('#status-sprint_id').val()
            ...
            if(status_id  == 'undefined'){
                ...
                $.ajax({
                    type: "POST",
                    url: '../../haillify/booking/update/status',
                    headers: {'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')},
                    data:{status_id : status_id, delivery_id : delivery_id, sprint_id : sprint_id } ,
                    success: function (data) {
        ...
        $(document).on('click', '.un-assign', function(e) {
            ...
            var deliveryId = this.getAttribute('data-delivery-id');
            $('#sprint_id').val(sprintId);
            $('#un_assign_delivery_id').val(deliveryId);
            $('#un-assign').modal();
        });
        ...
        $('.UnAssignBooking').submit(function (event) {
            ...
            var data = new FormData();
            data.append('sprint_id', $('input[name=sprint_id]').val());
            data.append('delivery_id', $('input[name=un_assign_delivery_id]').val());
            // $('#ex10').modal('toggle');
            // process the form
            $(".loader").show();
            $(".loader-background").show();
