...
@section('title', 'Update Address Approval')
...
input.form-control.update-address-on-change.google-address {
    width: 100% !important;
...
$(".Status").click(function(){
    ...
    var address=currentRow.find("td:eq(2)").children().val();
    var resp_lat=currentRow.find("td:eq(2)").children().attr("data-lat");
    var resp_lng=currentRow.find("td:eq(2)").children().attr("data-lng");
    var resp_city=currentRow.find("td:eq(2)").children().attr("data-city");
    var resp_state=currentRow.find("td:eq(2)").children().attr("data-state");
    var resp_state_code=currentRow.find("td:eq(2)").children().attr("data-state-code");
    var resp_postal_code=currentRow.find("td:eq(2)").children().attr("data-postal-code");
    if(!resp_lat){
        resp_lat = currentRow.find("td:eq(2)").children().attr("data-old-lat");
    }
    if(!resp_lng){
        resp_lng = currentRow.find("td:eq(2)").children().attr("data-old-lng");
    }
    if(!resp_city){
        resp_city = currentRow.find("td:eq(2)").children().attr("data-old-city");
    }
    if(!resp_postal_code){
        resp_postal_code = currentRow.find("td:eq(2)").children().attr("data-old-postal-code");
    }
    if(!resp_state){
        resp_state = currentRow.find("td:eq(2)").children().attr("data-state-old");
    }
    if(!resp_state_code){
        resp_state_code = currentRow.find("td:eq(2)").children().attr("data-state-code-old");
    }
    //   var del_id = element.attr("data-id");
     console.log(resp_lat);
...
$('.decline').on('click',function(){
    var element = $(this);
    $('#del_tracking_id').val(''+element.attr('data-tracking-id'));
    $('#ex5').modal();
});
...
<div class="right_col" role="main">
        ...
        @if ($message = Session::get('success'))
            ...
            <div class="page-title">
                <div class="title_left amazon-text">
                    <!-- <h3>Address Update Approval<small></small></h3> -->
                </div>
            </div>
                            ...
                            <?php
                               ...
                               foreach ($data as $response){
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $response->sprint_id ?></td>
                                        <td>
                                            <input type="text"
                                                ...
                                                data-lat=""
                                                data-old-lat="{{$response->lat}}"
                                                data-lng=""
                                                data-old-lng="{{$response->lng}}"
                                                data-state=""
                                                data-state-old="{{$response->state}}"
                                                data-state-code=""
                                                data-state-code-old="{{$response->state_code}}"
                                                data-city=""
                                                data-old-city="{{$response->city}}"
                                                data-postal-code=""
                                                data-old-postal-code="{{$response->postal_code}}"
                                                data-old-val="{{$response->address}}"
                                        ...
                                        <td>
                                        <a class="Status btn orange-gradient"  data-id=''>Approve</a>
                                        <a class="decline btn red-gradient"  data-tracking-id='{{$response->tracking_id}}'>Decline</a>
                                        </td>
                                    </tr>
                                    <?php
                                } 
    ...
    <div id="ex4" class="modal" style="display: none">
        <div class='modal-dialog'>
            ...
            <form action="../../../trackingid/multiple/approve/btn" method="post">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
    ...
    <div id="ex5" class="modal" style="display: none">
        <div class='modal-dialog'>
            ...
            <div class='modal-content'>
                ...
                <form action="../../../trackingid/multiple/approve/del" method="post">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" id="del_tracking_id" name="del_tracking_id" value="">
                    <div class="form-group">
                        <p><b>Are you sure you want to decline request?</b></p>
                    </div>
                    <div class="form-group">
                    <button type="submit" class="btn green-gradient btn-xs" >Yes</button>
                    <button type="button" class="btn red-gradient btn-xs" data-dismiss="modal" >No</button>
