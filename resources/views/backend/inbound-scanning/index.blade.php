    ...
    <div class="right_col" role="main">
        ...
        <div class="">
            ...
            <div class="row"><!---row-->
                        ...
                        <h1 style="position: relative;padding-right: 116px;"><b>Inbound Scanning </b> 
                            <button   style="background-color: #c6dd38;position: absolute;right: 0px;" data-id='{{$id}}'class='btn btn clearOrder' >Clear Order </button>
                            <a href="{{route('display.invalid',$id)}}" target="_blank"><button style="background-color: #c6dd38;position: absolute;right: 105px;" class="btn btn allInvalids">All Invalids</button></a>
                            <span id="button1"><button style="background-color: #c6dd38;position: absolute;right: 208px;" class="btn btn allInvalids">Orders to Scan</button></span>
                                                  ...
                                                  if(data.data.exist==0)
                                                  {
                                                    ...
                                                    var table = $('#datatable').DataTable();
                                                                if ( !table.data().count() ) {
                                                                    ...
                                                                    tracking_id_exists = table.rows().data();
                                                                    tracking_id_exists_array = [];
                                                                    $.each( tracking_id_exists, function( key, tracking_id_check ) {
                                                                        tracking_id_exists_array.push(tracking_id_check[1]);
                                                                    });
                                                      ...
                                                      success: function (data) {
                                                          if(data['status_code']==200)
                                                          {
                                                              ...
                                                              if(data.data.exist==0)
                                                              {
                                                                ...
                                                                //   // $('<tr>', { html:  }).appendTo($("#datatable"));
                                                                //   // var table_check = $('#datatable').DataTable()
                                                                //   // var tracking_id_table = table_check.search(value.tracking_id).row({search: 'applied'}).data();
                                                                //   // console.log(tracking_id_table);
                                                                //   // if (typeof tracking_id_table[1] === 'undefined'){
                                                                //   $('#datatable').DataTable().row.add(formatItem(value) ).draw();
                                                                //   $('#file_total_order').html(parseInt($('#file_total_order').html())+1);
                                                                //   $('#file_total_valid_order').html(parseInt($('#file_total_valid_order').html())+1);
