...
for($j=0;$j<count($counts);$j++) {
    ...
    echo "<td><button class='details green-gradient btn' data-route-id=" . $counts[$j]->route_id . " data-joey-id=" . $counts[$j]->joey_id . " title='Details'>D</button>
     <a class=' orange-gradient btn' target='_blank' href='../route/" . $counts[$j]->route_id . "/edit/hub/20' title='Edit Route'>E</a> 
...
function fullmap() {
    ...
    <?php    if(empty($_REQUEST['date'])){
                ...
                $.ajax({
                    ...
                    url : '../../backend/allroute/476674/location/joey?date=<?php echo $date; ?>',
                    type : 'GET',
                    dataType:'json',
                    success : function(data) {
                        console.log(data);
