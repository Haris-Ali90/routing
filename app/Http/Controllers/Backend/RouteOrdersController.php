...
class RouteOrdersController extends BackendController
{
  ...
  public function get_multipletrackingid(Request $request)
  {
        ...
        if(!empty($tracking_ids) || !empty($merchant_order_no) || !empty($phone_no) )
        {
          ...
          $orders=$user->orderBy('merchantids.id','DESC')
             ->get(array("sprint__sprints.id",'sprint__sprints.in_hub_route','sprint__sprints.creator_id','sprint__sprints.status_id',\DB::raw("CONVERT_TZ(sprint__sprints.created_at,'UTC','America/Toronto') as created_at"),'merchantids.tracking_id','merchantids.task_id','merchantids.merchant_order_num','sprint__contacts.phone','locations.address','locations.suite','merchantids.address_line2'));
                $i=0;
          ...
          return backend_view('multiplerouteorder',['data'=>$orders]);
  }
