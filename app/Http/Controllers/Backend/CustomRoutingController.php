...
class CustomRoutingController extends BackendController
{
    ...
    {
        $user= Auth::user();
        $id= CustomRoutingTrackingId::where('user_id','=',$user->id)->where('is_big_box','=',0)->where('tracking_id','=',$request->Tracking_id)
            ->where('is_inbound','=',0)
    ...
    public function getIndex($id,Request $request)
    {
        ...
        {
            $vendor=Vendor::
            where('id',477260)->get(['name','id']);
        }
        else if($id==19)
        {
            $vendor=Vendor::
            whereIn('id',[477282,476592,477340,477341,477342,477343,477344,477345,477346,477631,477629,477693])->get(['name','id']);
        }
        else if($id==20)
        {
            $vendor=Vendor::
            where('id',476674)->get(['name','id']);
        }
        else if($id==129){
            $vendor=Vendor::
            whereIn('id',[477607,477609,477613,477589,477641])->get(['name','id']);
        }
        else
        {
            $vendor=Vendor::
            ...
            477302,477303,477304,477305,477306,477296,477290,477297,477298,477299,477300,477320,477301,477318,477171,477559,477625,477587,477621,477627,477635,477633,477518,477661])->get(['name','id']);
        }
    ...
    {
        $user= Auth::user();
        $id= CustomRoutingTrackingId::where('user_id','=',$user->id)->where('is_big_box','=',0)->where('tracking_id','=',$request->Tracking_id)->where('is_inbound','=',0)->whereNull('deleted_at')->first();
    ...
    {
        $user= Auth::user();
         CustomRoutingTrackingId::where('user_id','=',$user->id)->where('is_big_box','=',0)->where('is_inbound','=',0)
         ->where('valid_id','=',1)->whereIn('tracking_id',$request->deleteId)->whereNull('deleted_at')->update(['deleted_at'=>date('Y-m-d H:i:s')]);
    ...
    public function postCreateRoute(Request $request)
    {
        ...
        //  $long[1] = substr($sprint->longitude, 3);
         $longitude= $sprint->longitude/10000000;
    ...
    public function postCreateOrder(Request $request)
    {
        ...
        $data['address'] =trim(str_replace(array("\n", "\r"), '', $request->address));
         ...
         $data['is_inbound'] =trim(str_replace(array("\n", "\r"), '', $request->is_inbound));
        ...
        //   $sprint->merchant_order_num= $data->merchant_order_num;
          ...
          if($response['http']['code']==400)
            ...
            ,'sprint__contacts.name','sprint__contacts.phone','merchantids.tracking_id','merchantids.task_id','sprint__tasks.sprint_id']);
                if($data['is_inbound'] == 1){
                    $tracking_id_data->is_inbound=1;
                }
                else{
                    $tracking_id_data->is_inbound=0;
                }
                ...
                477298,477299,477300,477320,477301,477318,477328,476294,477334,477335,477336,477337,477338,477339,477171,477559,477625,477607,477609,477613,477587,477621,477627,477631,477629,477589,477641,477635,477633,477518,477661]))
                 {
    ...
    public function getTrackingIdDetail(Request $request)
    {
        ...
        $user= Auth::user();
        $exist=1;
        $tracking_id_data=CustomRoutingTrackingId::where('user_id','=',$user->id)->where('is_big_box','=',0)->where('tracking_id','=',trim($track))->
        where('hub_id','=',trim($request->hub_id))->where('is_inbound','=',0)->whereNull('deleted_at')->first();
        ...
        //Mark:- Order Exist condition ...
         if($tracking_id !=null)
         {
            ...
            elseif($request->hub_id==157)
            {
             ...
             whereIn('status_id',[136,106,110,102,112,137,107,131,135,142,17,133,121,113,114,116,117,118,132,138,139,144,143,105,111,108,109,146,124])->OrderBy('id','DESC')->
             first();
            //  dd($tracking_id,$checkReturnDeliveredStatus);
            ...
            Sprint::where('id','=',$tracking_id->id)->update(['in_hub_route'=>0]);
             ...
             return response()->json( ['status_code'=>200,"data"=>["tracking_id"=>trim($track),"route_enable_date"=>$tracking_id_data->route_enable_date,"vendor_id"=>$tracking_id->creator_id,
         ...
         else
         {
             ...
             //Mark:- Order Does not exist condition ...
    ...
    public function checkTrackingIdStatus(Request $request)
    {
        ...
        $tracking_id = JoeyRouteLocations::join('merchantids','merchantids.task_id','=','joey_route_locations.task_id')
            ...
            ->pluck('sprint__tasks.status_id as id');

        $data["count"] = count($tracking_id);
        $data["response_data"] = $tracking_id;
    ...
    public function editOrder(Request $request)
    {
        ...
        $latitude_int = $googleAddress['lat']*1000000;
        $longitude_int = $googleAddress['lng']*1000000;

        $locationData->latitude =(int)$latitude_int;
        $locationData->longitude =(int)$longitude_int;
        $locationData->save();
    ...
    public function updateStatuscreateReattempt(Request $request)
    {
        ...
        $borderlessData = BoradlessDashboard::where('sprint_id', $merchantIds->sprint_id)->where('task_id',$merchantIds->task_id)->whereNull('deleted_at')->first();

        if (!empty($borderlessData))
        {
            //update boradless_dashboard for return order
            $borderless_update = BoradlessDashboard::where('sprint_id', $merchantIds->sprint_id)->where('task_id',$merchantIds->task_id)
                ->update([
