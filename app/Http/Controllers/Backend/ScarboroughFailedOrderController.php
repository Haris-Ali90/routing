...
class ScarboroughFailedOrderController extends BackendController
{
    ...
    public function getAllBorderlessFailedOrder(Request $request)
    {
        ...
        $borderless_vendor_ids=[477542,477559,477607,477587,477589,477641,477518,477621,477625];
        $data = $request->all();
        if($request->vendor_id==null &&  $request->tracking_id==null){
            ...
            if (!empty($data['vendor_id'])) {
                ...
                {
                    $data=[];
                    return backend_view( 'testingfailedorder.scarborough-failed-order', compact('data','title') );
                }
                $failedorders = $failedorders->where('vendor_id', '=', $data['vendor_id']);
            }
            if (!empty($data['tracking_id'])) {
