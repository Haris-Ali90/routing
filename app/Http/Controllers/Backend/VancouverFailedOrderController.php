...
class VancouverFailedOrderController extends BackendController{

    public function getAllBorderlessFailedOrder(Request $request)
    {
        $title="Vancouver Failed Orders";
        // $borderless_vendor_ids=[477542,477559,477607,477587,477589,477641,477518];
        $borderless_vendor_ids=[477613,477589,477641,477607,477635];
        $data = $request->all();
        if(($request->vendor_id==null &&  $request->tracking_id==null) || ($request->vendor_id=='' &&  $request->tracking_id=='')){
            ...
            if (!empty($data['vendor_id'])) {
                ...
                {
                    $data=[];
                    return backend_view( 'testingfailedorder.vancouver-failed-order', compact('data','title') );
                }
